...
- 先頭の主要キロ程は、対応する 2 指標を 1 行にまとめて表示する。
  - 総営業キロと JR 計算キロ
  - JR 北海道の営業キロと計算キロ
  - JR 線営業キロと会社線営業キロ
- JR 北海道 / 東日本 / 四国 / 九州は、営業キロと計算キロを地域ごとに 1 行へまとめる。
- 計算キロが営業キロと同値の組み合わせでは、計算キロ側を省略する。
- 会社線営業キロと BRT 営業キロがともにない場合、先頭行のラベルは `営業キロ / 計算キロ(JR)` ではなく `営業キロ / 計算キロ` とする。
- 会社線営業キロと BRT 営業キロがともにない場合、JR 営業キロ単独行は表示しない。
...
- 0 円または値なしの項目は原則省略する。
- IC 運賃がある場合、`普通運賃（IC運賃）` の 1 行に通常運賃と IC 運賃を併記し、IC 運賃の独立行は出さない。
- 普通運賃と往復運賃は、会社線運賃があれば 1 行 2 値の形式で表示する。
- 往復運賃は、会社線の補助値がない場合でも金額を行の右端へ配置する。
- 小児運賃 / 小児往復運賃、学割運賃 / 学割往復運賃も、普通運賃と同様に 1 行 2 値で右端寄せする。
- 小児・学割の往復欄は画面幅に応じた 2 列分割へは崩さない。
...
- 補足文言は以下の優先順で決める。
  1. 有効日数（`ticketAvailDays`）が 1 日以下 → 「途中下車前途無効」
     - 営業キロでは判定しない。大都市近郊区間内相互発着は 101km 以上でも当日限り・途中下車不可のため、
       WASM コア（`1 < getTicketAvailDays()`）と同じ基準に揃える。
  2. 都区市内発着
  3. 通常の途中下車可
- 近郊区間の注記（「近郊区間内ですので最安運賃の経路にしました(途中下車不可、有効日数当日限り)」等）は
  WASM が `messages` に含めて返すので、画面側では判定せず `messages` をそのまま表示する。
  `isSpecificFare` は特定区間割引運賃のフラグであり、近郊区間の判定には使わない。
