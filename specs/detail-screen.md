...
- PWA では既存キャッシュ上の WASM 資産で起動でき、オフライン時も共有 URL から詳細を復元できるようにする。
...
### 上部バーの発着
- 上部バーの発着表示は、入力経路そのものの始点と終点を表す。
- `departureStationName()` / `arrivevalStationName()` を正本とする。
- 旅客営業規則第86・87条や都区市内発着など、運賃計算上の読替後の駅名には置き換えない。

### 区間カードの発着
- 区間カードは「その運賃計算結果がどの発着として扱われたか」を表す。
- `FareInfo.beginStation` / `FareInfo.endStation` を正本とする。
- `FareInfo.beginStation` / `FareInfo.endStation` があれば、旅客営業規則第86・87条適用後の発着名を含め、運賃計算上の発着をそのまま表示する。
- `FareInfo.beginStation` / `FareInfo.endStation` が空の場合だけ、入力経路の発駅 / 着駅へフォールバックする。
- したがって、上部バーと区間カードで発着表示が異なることを許容する。
- この差分は誤表示ではなく、「入力経路の端点」と「運賃計算上の発着」の違いを示すものとして扱う。
...
## オフライン前提
- `/detail` は deep link されやすいため、既訪問時は shell fallback で画面復元できることを前提にする。
