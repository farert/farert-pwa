...
-   入力ダイアログから経路テキストを直接貼り付け/入力してインポートします。
...
-   不正なインポート経路はそのエラー内容をアラートで表示する
    - `-200`, `-300` の場合、不正な駅名または路線名とともに、何ワード目かを表示する。
    - `failItem` 駅名または路線名、`offset` 0から数えて何ワード目かなの、"`offset+1` 番目のワード" なので、エラーメッセージの後ろに（）書きで表示する

### **[エクスポート]**

-   全ての保持経路をエクスポートします。
-   `WebShare API`またはクリップボードを使用して共有します。
-   エクスポート形式: 改行区切りのテキスト形式で、1経路1行のCSV。
