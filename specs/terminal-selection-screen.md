...
各ビューでリストされた項目は、"路線"、"駅" の2種類で、"路線"を選択した場合、さらに **<駅選択>** 画面へ遷移する。
...
- `都道府県` タブ表示中は、画面右下に `↑` / `↓` のフローティングボタンを表示する
