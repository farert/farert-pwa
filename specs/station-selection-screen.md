...
- ``prefecture`` も``group`` もなく、``station`` と ``line`` が指定され、``from=main``
  - 経路の発駅を基準に `getBranchStationsByLine(line, 発駅)` を呼び、分岐駅一覧を列挙する
  - 表示順は路線の営業キロ順とする
  - 現在駅（`station`）は同じリスト内で強調表示する
  - 経路の発駅が同一路線上に存在する場合は分岐駅一覧に含め、駅名の前に ``<発駅>`` を付けて表示する
- 画面タイトル == "着駅指定"
- ``prefecture`` も``group`` もなく、``station`` と ``line`` が指定され、``from=main``
  - 路線に属する駅一覧を列挙する
- '発着駅選択'
...
- 所属路線は `wasm.getLinesByStation(station)` で取得する
- `from=main` かつ `line` 指定時は、見出しで既に示している選択中路線 `line` を所属路線表示から除外する
  - 例: 画面タイトルが `分岐駅選択 - 北陸新幹線` のとき、2行目には `北陸新幹線` を含めない
...
- 駅一覧表示中は、画面右下に `↑` / `↓` のフローティングボタンを表示する
- ボタンは円形で、テーマカラーの紫系配色とする
- `↑` をタップすると画面最上部へスムーズスクロールする
- `↓` をタップすると画面最下部へスムーズスクロールする
...
- 交互に切り替える時のトランジションは、表示リスト全体が画面中央の縦軸を中心に回転するようなアニメーションにする。
...
- `from=main` で `addRoute()` の戻り値が `-1` の場合は、エラー文言を ``経路が重複しています`` とする
- `from=main` で `addRoute()` の戻り値が `0`, `4`, `5` の場合は追加自体は成功扱いとし、メイン画面に戻る
