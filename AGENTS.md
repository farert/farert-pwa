...
## 作業別スキル（.claude/skills/ — 該当作業の前に必ず読む）
- `verify` — テスト・lint・型チェックの実行と判定（Docker 専用ツールチェーンの注意含む）
- `run` — dev サーバ起動・ポート・SW キャッシュのトラブルシュート
- `update-wasm` — WASM の修正〜リビルド〜PWA 反映のクロスリポジトリ手順
- `fare-logic` — 運賃・経路コードの共通ユーティリティと必須実装パターン
